...
public class TransactionFinanceService : ITransactionFinanceService
{
  ...
  public async Task<Result<List<TransactionDataDto>>> GetTransactionItemsByAsyncForDate(DateTime StartDate, DateTime EndDate, DateType dateType, int userId, TransactionType transactionType, CancellationToken cancellationToken = default)
  {
    switch (dateType)
    {
      case DateType.Day:
        var itemByDay = await _repository.ListAsync(new ItemsByDaySpec(userId, transactionType,  StartDate,  EndDate), cancellationToken);
        return GetListTransactionByDay(itemByDay);

      case DateType.Week:
        var itemByWeek = await _repository.ListAsync(new ItemsByWeekSpec(userId, transactionType, StartDate, EndDate), cancellationToken);
        return GetListTransactionByWeek(itemByWeek);

      case DateType.Month:
        var itemByMonth = await _repository.ListAsync(new ItemsByMonthSpec(userId, transactionType, StartDate, EndDate), cancellationToken);
